...
// RestoreTableOptions are the options to pass to Restore a specific Table
type RestoreTableOptions struct {
	Src              string
	Keyspace         string
	Table            string
	Tag              string
...
// RestoreTable performs restore of a specific table
func RestoreTable(o RestoreTableOptions) error {
	log.Println("Restore table started!")
	srcPrefix, srcBasePath := utils.SplitInTwo(o.Src, "://")
	...
	log.Println("Calculating paths. This may take a while...")
	srcPath := filepath.Join(srcBasePath, o.Keyspace, sum, o.Tag)
	fromToPaths, podsToBeRestored, tablesToRefresh, err := utils.GetFromAndToPathsSrcTableToK8s(srcClient, k8sClient, srcPrefix, srcPath, srcBasePath, o.Namespace, o.Container, o.CassandraDataDir, "/"+o.Table+"/")
